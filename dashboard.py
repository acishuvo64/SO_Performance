...
def load_workbook(file_bytes, file_key):
    ...
    # ---- Attendance cleaning ----
    att["Attendance Date"] = pd.to_datetime(
        att["Attendance Date"]).dt.normalize()
    ...
    att["Total Hours (min)"] = att["Total Hours"].map(
        parse_total_hours_to_minutes)
...
def build_attendance_summary(att):
    ...
    summary["Late_%"] = (summary["Late_Days"] /
                         summary["Active_Days"] * 100).round(1)
    summary["Avg_InTime"] = summary["Avg_InTime_min"].map(minutes_to_hhmm)
    summary["Avg_MarketStay"] = summary["Avg_MarketStay_min"].map(
        minutes_to_hm)
...
def build_activity_summary(so):
    ...
    summary["Target_Visits"] = summary["Visit_Active_Days"] * \
        DAILY_OUTLET_TARGET
...
if os.path.exists(LOGO_PATH):
    st.sidebar.markdown('<div class="sidebar-logo-card">',
                        unsafe_allow_html=True)
...
if os.path.exists(BADGE_PATH):
    ...
    st.sidebar.markdown(
        '<div class="sidebar-badge">LOGO</div>', unsafe_allow_html=True)

st.sidebar.markdown(
    '<div class="sidebar-filters-title">Dashboard Filters</div>', unsafe_allow_html=True)
...
if not os.path.exists(DATA_FILE_PATH):
    st.markdown(
        f'<div class="dash-header-title">{DASHBOARD_TITLE}</div>', unsafe_allow_html=True)
    st.markdown(
        f'<p class="dash-header-sub">{DASHBOARD_TAGLINE}</p>', unsafe_allow_html=True)
    st.info(
...
if start_date > end_date:
    st.sidebar.warning(
        "Start Date, End Date-er por hote parbe na — date range shodhon kora hoyeche.")
...
with st.sidebar.expander("More Filters (Group / Region / Zone / RSM)", expanded=False):
    ...
    zones_available = sorted(
        so_raw[so_raw["Region Name"].isin(
            sel_regions)]["Zone Name"].dropna().unique().tolist()
    )
    sel_zones = st.multiselect(
        "Zone", zones_available, default=zones_available)

    rsms_available = sorted(
        so_raw[so_raw["Zone Name"].isin(
            sel_zones)]["RSM"].dropna().unique().tolist()
    )
    sel_rsms = st.multiselect("RSM", rsms_available, default=rsms_available)

st.sidebar.caption(
    f"Last data update: {last_modified.strftime('%d %b %Y, %I:%M %p')}")
...
if filtered_staff_ids:
    att = att[att["Staff Id"].isin(
        filtered_staff_ids) | att["Staff Id"].notna()]
...
report = report[report["StaffID"].isin(
    filtered_staff_ids)].reset_index(drop=True)
...
with h_right:
    if os.path.exists(BADGE_PATH):
        ...
        st.markdown('<div class="dash-header-badge">LOGO</div>',
                    unsafe_allow_html=True)
...
kpi_late_pct = f"{report['Late_%'].mean():.1f}%" if report["Late_%"].notna(
).any() else "-"
kpi_achv_pct = (
    f"{report['Achievement_%'].mean():.1f}%" if report["Achievement_%"].notna(
    ).any() else "-"
)
...
st.markdown('<div class="section-title">Overall Summary</div>',
            unsafe_allow_html=True)
...
with tabs[0]:
    ...
    with c2:
        grp_late = report.groupby("SubBusiness", as_index=False)[
            "Late_%"].mean()
        fig2 = px.bar(
            grp_late,
            x="SubBusiness",
...
with tabs[1]:
    ...
    c1, c2 = st.columns(2)
    with c1:
        plot_df = report.dropna(
            subset=["Avg_InTime_min"]).sort_values("Avg_InTime_min")
        fig = px.bar(
            plot_df.head(20),
            x="Avg_InTime_min",
            y="SR_Name",
...
with tabs[2]:
    ...
    with c1:
        ...
        if good.empty:
            good = report.sort_values(
                "Avg_Visit_Per_Day", ascending=False).head(10)
            st.caption(
                "(Kew exact target chhuiynai — tai overall top 10 dekhano hocche)")
        fig = px.bar(
            good,
            x="Avg_Visit_Per_Day",
            y="SR_Name",
    ...
    with c2:
        ...
        st.caption(
            f"Criteria: Avg Outlet Visit/day < {BAD_PERFORMER_OUTLET_LIMIT}")
        ...
        if bad.empty:
            ...
            st.caption(
                "(Kew threshold er niche na — tai overall bottom 10 dekhano hocche)")
        fig2 = px.bar(
            bad,
            x="Avg_Visit_Per_Day",
            y="SR_Name",
...
with tabs[3]:
    ...
    c1, c2 = st.columns(2)
    with c1:
        grp_visit = so.groupby("SubBusiness", as_index=False)[
            "Number Of Outlet Visit"].sum()
        fig = px.pie(
            grp_visit,
            names="SubBusiness",
        ...
        st.plotly_chart(fig, width='stretch')
    with c2:
        grp_ach = report.groupby("SubBusiness", as_index=False)[
            "Achievement_%"].mean()
        fig2 = px.bar(
            grp_ach,
            x="SubBusiness",
    ...
    grp_pick = st.selectbox("Group select korun",
                            groups, key="grp_chart_select")
...
with tabs[4]:
    ...
    idx = route_day.groupby(["Staff ID", "Weekday"])[
        "Number Of Outlet Visit"].idxmax()
    ...
    market_visits = (
        ...
        .agg(
            Visit_Dates=("Visit Date", lambda s: ", ".join(
                sorted(d.strftime("%d-%b") for d in s.unique()))),
            Visit_Count=("Visit Date", "nunique"),
            Actual_Weekdays=(
                "Weekday", lambda s: ", ".join(sorted(s.unique()))),
        )
        .reset_index()
    )
    ...
    if repeat_markets.empty:
        ...
        repeat_markets["Day_Mismatch"] = repeat_markets.apply(
            _check_mismatch, axis=1)
